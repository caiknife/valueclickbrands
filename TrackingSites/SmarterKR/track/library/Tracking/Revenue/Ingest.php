...
class Tracking_Revenue_Ingest extends Tracking_Revenue_Abstract
{
    ...
    public function getTracker ()
    {
        if (self::$_tracker == null) {
            ...
            self::$_tracker->addAccount("UA-40418033-1");
        }
    ...
    public function GALogger ()
    {
        ...
        if (empty($ad_category) || empty($ad_action)) {
            $sessionID = $this->_session->getSessionId();
            $this->logError("GARI category and action is empty.(revPartner is $revPartner and revPartnerType is $revPartnerType and sessionID is $sessionID)");
            die();
        }
