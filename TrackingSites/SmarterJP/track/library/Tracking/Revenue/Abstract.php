...
abstract class Tracking_Revenue_Abstract extends Tracking_Request_Abstract
{
    ...
    public function __construct ()
    {
        ...
        if (preg_match('/^([a-z0-9]*)?(demo|dev|beta)[0-9]*\./i', $this->getServer('HTTP_HOST'))) {
            $this->_debug = true;
        } else {
            if ($this->getParam('track_traffic') === 'test') {
                $this->_debug = true;
            }
        }
        
        if (is_file(TRACKING_ROOT_PATH . '/config/GA.ini')) {
            $this->_GAParams = parse_ini_file(TRACKING_ROOT_PATH . '/config/GA.ini', true);
        } else {
            $message = TRACKING_ROOT_PATH . '/config/GA.ini file does not exist.';
        ...
        //$this->_landingReferral = $this->_session->getLandingReferer();//csus
        $this->_landingReferral = $this->_session->getReferer();//smjp
        //$this->set_sourceAbbr();
        
        if ($this->_debug == true) {
            // $this->pre($this->_GAParams);
            $this->vd('source value is:' . $this->_source);
            //$this->vd('_sourceAbbr value is:' . $this->_sourceAbbr);
            $this->vd('landingreferral value is:' . $this->_landingReferral);
        }
    }

    function set_sourceAbbr ()
    {
        if ($this->_source != null) {
            foreach ($this->_GAParams['traffic_partner'] as $key => $val) {
                if (stripos($this->_source, $key) !== false) {
    ...
    function pre ($x, $y = null, $color = '#000000')
    {
        if ($this->_debug == true) {
    ...
    function vd ($x, $y = null, $color = '#000000')
    {
        if ($this->_debug == true) {
            echo '<pre style="color:' . $color . '">';
            if ($y) {
    ...
    {
        return preg_replace('/^([a-z0-9]*)?(demo|dev|beta|www)[0-9]*\./i', '', $this->getServer('HTTP_HOST'));
    }
