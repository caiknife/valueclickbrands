 ...
 * @version    CVS: $Id: Logger.php,v 1.1 2013/06/27 07:54:18 jjiang Exp $
 */
            ...
            foreach ($data as $events) {
                ...
                if (is_array($params)) {
                    ...
                    if ('PAGEVISIT' == $method) {
                        ...
                        if ($productType = $session->getProductType()) {
                            $params['testKey']    = $productType;
                        }
                        $trafficType = 'other';
                        if ($session->isMobileTraffic()) {
                            $trafficType = 'mobile';
                        }
                        $params['testKey'] = $params['testKey'] . '|' . $trafficType;
                        $params['httpStatus']    = $responseCode;
                    }
                }
                $log->$method($params);
