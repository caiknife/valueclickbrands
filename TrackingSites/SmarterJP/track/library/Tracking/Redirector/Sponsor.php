 ...
 * @version    CVS: $Id: Sponsor.php,v 1.1 2013/06/27 07:54:18 jjiang Exp $
 */
...
class Tracking_Redirector_Sponsor extends Tracking_Redirector_Abstract
{
    ...
    const G_STS_BASE_URL    = TAG_SERVICE_BASE_URL;
    ...
    private function _requestChannelTag($siteId, $keyword, $source, $country)
    {
        ...
        $requestHost = rtrim(self::G_STS_BASE_URL, '/') . '/setgtag/?';
        $requestUrl  = $requestHost . http_build_query($query);

        if ($this->_request->getCookie('debug')=='yes') {
            $requestType = ($this->_sponsorType == Tracking_Constant::SPONSOR_GOOGLE) ? 'G-STS' : 'Y-STS';
