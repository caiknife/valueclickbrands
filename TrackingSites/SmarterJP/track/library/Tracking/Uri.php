 ...
 * @version    CVS: $Id: Uri.php,v 1.1 2013/06/27 07:54:18 jjiang Exp $
 */
...
class Tracking_Uri
{
    ...
    public static $encodings = array(
        ...
        self::SPONSOR_TYPE, self::COUPON_BEACON,
    );
     ...
     * @return string
     */
    public static function buildCouponUrl ($aParam = array())
    {
        if (!isset($aParam['m'])) {
            return '/';
        }
            // $params[self::BUILD_TYPE] = 'retarget';
            
        $aParam['requestid'] = Tracking_Session::getInstance()->getRequestId();
        $aParam['from'] = 'smjp';
        $query = http_build_query($aParam);
        $params = array();
        $params[self::DESTINED_URL] = 'http://www.couponmountain.com/stats/redir.php?' . $query;
