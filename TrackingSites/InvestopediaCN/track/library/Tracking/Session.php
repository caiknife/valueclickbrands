...
class Tracking_Session
{
     ...
     * trafficType, source,sourceGroup, ui, requestId
     */
    ...
    {
        $roughReferer = trim($this->_getValueFrom($this->_data, self::COOKIE_DATA_LANDING_REFERER));
        if ($roughReferer == null || $roughReferer == "") {
            return '';
        }
        return (string) @gzinflate(base64_decode($roughReferer));
    }
    ...
    {
        $roughLandingUri = trim($this->_getValueFrom($this->_data, self::COOKIE_DATA_LANDING_URI));
        if ($roughLandingUri == null || $roughLandingUri == "") {
            return '';
        }
        $rLandingUrl = base64_decode($roughLandingUri);
        return (string) @gzinflate($rLandingUrl);
    }
    ...
    public function update ()
    {
        ...
        foreach ($data as $name => $value) {
            if (0 === strpos($name, self::COOKIE_PREFIX)) {
                ...
                $life = ($name == self::COOKIE_USER_ID) ? 2 * 3600 * 24 * 365 : self::$_cookieLife;
                if ($life) {
                    $life += time();
                }
    ...
    public function getMemcacheNode ($cacheName)
    {
        if ($this->_memcache == NULL) {
            $this->_memcache = Zend_Registry::get("cache");
            if ($this->_memcache == NULL) {
                return false;
            }
        }
        $item = $this->_memcache->load($cacheName);
        return $item;
    }

    public function setMemcacheNode ($cacheName, $increment, $reset = "no")
    {
        if ($this->_memcache == NULL) {
            $this->_memcache = Zend_Registry::get("cache");
        }
        ...
        // cache last for one week
        $this->_memcache->save($item, $cacheName, array(), 2592000, 10);
        return TRUE;
    }
