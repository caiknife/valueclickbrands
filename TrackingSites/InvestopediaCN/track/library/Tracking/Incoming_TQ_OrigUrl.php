 ...
 * @version    CVS: $Id: Incoming_TQ_OrigUrl.php,v 1.1 2013/07/10 01:34:45 jjiang Exp $
 */
...
class Tracking_Incoming
{
    ...
    public function run ()
    {
        ...
        // ENDTODO
        // 鍦ㄦ祴璇昑Q姝ｇ‘鎬т箣鍓嶏紝鎶奍ncoming瑕佽褰曚笅鏉ワ紝纭濂戒互鍚庯紝濡傛灉涓嶆槸鏂皊ession锛屽彲浠ヨ烦杩噄ncoming logging銆�
        if ($this->_session->isLanding()) {
            ...
            $this->_session->setLandingReferer($this->_request->getHttpReferer())
                ->setLandingUri($this->_request->toString());
        ...
        // TODO
        // call TQ service with PV parameters. Change Valid and session id is they are changed. And source tag ... returned by TQ service.
        // _callTQService
        // if is new session then go on with islanding process
        // else log pv only
        // $rs = $this->_callTQService($callService); //call TQ service only but not log pv.
        $res = $this->_callTQService(); // call TQ service only but not log pv.
        
        if ($res != null && sizeof($res) > 0) {
            if ($res['pvValidKey'] !== '' && $res['pvValidKey'] >= 0) {
                $this->_session->setTrafficType($res['pvValidKey']);
                
                if ($res['sessionid'] != $res['original_sessionid'] && $res['sessionid'] !== '') {
                    $this->_session->setSessionId($res['sessionid']);
                    $this->_session->setIsLanding(false);
                } else {
                    if ($res['incomingValidKey'] !== '' && $res['incomingValidKey'] >= 0) {
                        $this->_session->setIsLanding(true);
                    }
                }
            }
