<?php
/** Google Analytics Module by Analytics Pros.
 ...
 *        ... fixed several warnings where built-in function references 
 *        are passed as constants, require strings
 ...
 */
function iipre($x){
    echo '<pre style="font-weight:bold;font-size:18px;color:green">';
    print_r($x);
    echo '</pre>';
}
function iivd($x){
    echo '<pre style="font-weight:bold;font-size:18px;color:red">';
    var_dump($x);
    echo '</pre>';
}
...
if (PHP_VERSION_ID < 50200 || !class_exists('DateTimeZone')){
...
if (PHP_VERSION_ID < 50200 || !class_exists('DateTime')) {
  class DateTime {
...
class GoogleAnalyticsTracker {
  ...
  private function run_queue($clear = true){
    ...
    iivd('full_auto:'.$this->full_auto);
    iivd('headers_sent:'.!headers_sent());
    iivd('send_cookies:'.$this->send_cookies);
  ...
  public static function batch_request($urls, $useragent = null){
    for($i = 0; $i < count($urls); $i++){
      if(self::$send){
          iipre($urls[$i]);
          GAURLRequest::curl($urls[$i], $useragent);
      } 
    }
  }
