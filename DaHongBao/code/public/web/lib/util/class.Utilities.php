...
//		self::$filter = array(ord('.')=>1, ord(',')=>1);
	 ...
	 * @param $len 截取的长度
	 */
	public static function cutString($str, $len, $tail='') {
	...
	public static function getTagSrcForDiscount($string, $maxCount=5) {
		$tagstring = "";
		if($string!=""){
			$tagarray = explode(",",$string);
			$count = 0;
	 ...
	 * tag format
	 */
	public static function getTagSrcForSearch($string,$highstring="", $maxCount=5) {
		$tagstring = "";
		if($string!=""){
			$tagarray = explode(",",$string);
			$count = 0;
	...
	function setArrayCache($file, $data) {
		$common_channel = var_export($data, true);
		$common_channel = "<?php return " . $common_channel . '; ?'.'>';
		if(file_exists(dirname($file)) == false) {
			mkdir(dirname($file), 0755, true);
		}
		file_put_contents($file, $common_channel);
		return true;
	}
	...
	static function filterHtmlCode($data) {
		if ($data) {
			$data = str_replace("\s+", " ", strip_tags(trim($data)));
		}
		return  $data;
	}
	 ...
	 * 计算分页
	 */
	public static function getPageStr($params, $pageUrl, $count, $pageSize=10) {
		$totalCount = $count;
		if($params['pn'] > 0) {
			$pageno = $params['pn'];
		} else {
		...
		if($pageno > 1) {
			$params['pn'] = $pageno - 1;
			$url = self::getPageUrl($pageUrl, $params);
	...
	public static function getPageUrl($pageUrl, $params) {
		if ($params["pn"]) {
			$pageUrl = str_replace('%d', $params["pn"], $pageUrl);
		}
		return $pageUrl;
	}
