...
if(empty($coupcat) && !empty($_GET['id'])){
	redirect301("/bbs/read.php?tid=".$_GET['id']);
   	exit;
}
...
$cl = $phpwind->addforumclick($_GET['id']);
$a = $phpwind->getforumdetail($_GET['id']);

$other = $phpwind->getlastandnext($_GET['id'],$cid);
...
$a[0]['content'] = str_replace("\n","<br />",$a[0]['content']);
preg_match_all("/attachment=(\d+)\]/",$a[0]['content'],$bb);
foreach($bb[1] as $key=>$value){
	//echo $value;
	$imagesrc = $phpwind->getImgSrc($value);

	$str = __IMAGE_SRC."bbs/attachment/".$imagesrc;
	$a[0]['content'] = preg_replace("/\[attachment=(\d+)\]/","<img src=".$str.">",$a[0]['content'],1);
}
...
$b = $phpwind->getforumreview($_GET['id']);
foreach ($b as $key=>$value){
	$b[$key]['postdate'] = get_date($value['postdate']);
	//echo $value['postdate'];

}
	$pageCount = ceil(count($b)/ 10);
...
if($a[0]['type']!=0){
	$tpl->assign('showtopic',"1");
	$typeid = $a[0]['type'];
	$topicrow = $phpwind->getTopicRowById($typeid);
	$tpl->assign('topicname',$topicrow['title']);
	$tpl->assign('topicid',$typeid);
