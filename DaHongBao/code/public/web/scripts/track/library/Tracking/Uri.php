 ...
 * @version    CVS: $Id: Uri.php,v 1.1 2013/04/15 10:58:19 rock Exp $
 */
...
class Tracking_Uri
{
    ...
    public static function build($params, $key=NULL)
    {
        ...
        if (!empty($params[self::ASYNC_TYPE])) {
            $script = $config->script->async;
        } else if (!empty($params[self::BUILD_TYPE])) {
            $script = $config->script->redir;
        } else {
            $script = '';
        }
    ...
    /**
     * build coupon url,redirect to CMUS and then to merchant site
     *
     * @param string $merchant_url            
     * @param string $destined_url            
     * @return string
     */
    public static function buildCouponUrl ($merchant_url = null, $destined_url = null)
    {
        if ($merchant_url == null) {
            return '/';
        }
        
        if ($destined_url == null) {
        ...
        $aParam['from'] = 'dhb';
        $aParam['rdtp'] = 'DHB';
        $aParam['url'] = base64_encode($merchant_url);
