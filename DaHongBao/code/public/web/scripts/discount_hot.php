...
$tpl = new sTemplate();
$tpl->setTemplate("new/discount_hot.htm");
...
$disountcacheid = "disounthotcacheid".$pageid."-".$_COOKIE['cityid'];
if ($list1 = $cache->get($disountcacheid)) {
	$list1 = unserialize($list1);//use cache
} else {
	$list1 = $odiscount->getHotDiscountList($_COOKIE['cityid'],$pageid);
...
$countpage = $odiscount->getHotDiscountCount($_COOKIE['cityid']);
...
$pageStr = getPageStrs($countpage,10,($pageid-1)*10,'discount_hot','Pg','');
