...
if($_COOKIE['cityid']==2){
	$mercityid = "21";
}else if($_COOKIE['cityid']==1){
	$mercityid = "10";
}else{
	$mercityid = "";
}

if(isset($_GET['mercityid'])){
	$mercityid = $_GET['mercityid'];
	$addressarray = $merchant->getMerchantAddress($id,$mercityid,"1",$_GET['areaid']);
}else{
	$addressarray = $merchant->getMerchantAddress($id,$mercityid,"");
}
			...
			for($i=0; $i<count($couponList); $i++) {
				if($couponList[$i]["isFree"] == '0') {
					...
					if(($couponList[$i]["isMore"] == 0 || $couponList[$i]["hasDetail"] == 0) && $couponList[$i]["ImageDownload"] != 1) {
						...
						$couponInfo[$i]["url"] = Utilities::getURL("couponFree", array("NameURL" => $merchantinfo['NameURL'],
				                        "Coupon_" => $couponList[$i]["Coupon_"]));
					}
