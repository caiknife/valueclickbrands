...
if(!$action || $action=='digest' || $action=='poll' || $action=='sale'){
	...
	$threaddb=array();
	$query=$db->query("SELECT tid,fid,subject,postdate,lastpost,replies,hits,digest,titlefont FROM pw_threads WHERE authorid='$winduid' AND fid!='$db_recycle' AND dhbid='' $sql ORDER BY postdate DESC $limit");
	...
	$postdb=array();
	$query=$db->query("SELECT p.pid,p.postdate,t.tid,t.fid,t.subject,t.authorid,t.author,t.titlefont,t.digest FROM $pw_posts p LEFT JOIN pw_threads t USING(tid) WHERE p.authorid='$winduid' AND p.fid!='$db_recycle' AND t.dhbid='' ORDER BY p.postdate DESC $limit");
	while($rt=$db->fetch_array($query)){
		if ($rt['titlefont']){
			$titledetail=explode("~",$rt['titlefont']);
			if($titledetail[0])$rt['subject']="<font color=\"$titledetail[0]\">$rt[subject]</font>";
