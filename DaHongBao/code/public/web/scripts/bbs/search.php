	...
	if($gp_allowsearch==2){
		...
		if($db_plist){
			...
			foreach($p_list as $key=>$val){
				$p_table .= "<option value=\"$val\">posts$val</option>";
			}
			$p_table .= "</select>";
		}
		if($db_tlist){
	...
	if(isset($_GET['sid']) && $_GET['sid']){
		...
		if(empty($schedid)){
			...
			if(($keyword || $pwuser || !is_numeric($authorid)) && $gp_searchtime != 0){
				if($timestamp - $winddb['lastpost'] < $gp_searchtime){
					Showmsg('search_limit');
				}
				$sqladd  = '';
				$tdtime >= $winddb['lastpost'] && $sqladd .= ",todaypost=0";
				$montime>= $winddb['lastpost'] && $sqladd .= ",monthpost=0";
				$db->update("UPDATE pw_memberdata SET lastpost='$timestamp' $sqladd WHERE uid='$winduid'");
			}
			if(is_numeric($seekfid)){
