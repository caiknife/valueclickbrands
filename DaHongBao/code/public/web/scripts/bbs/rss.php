...
if(time()-@filemtime($cache_path) > $Rss_updatetime*60){
	...
	if($fid){
		...
		$sql="WHERE t.fid='$fid' AND ifcheck=1 AND topped='0' ORDER BY lastpost DESC LIMIT $Rss_listnum";
	} else{
		$fids=$extra='';
		...
		$sql="WHERE fid IN($fids) AND ifcheck=1 AND topped='0' ORDER BY lastpost DESC LIMIT $Rss_newnum";
	}
	...
	$query=$db->query("SELECT t.tid,t.fid,t.subject,t.author,t.postdate,tm.content FROM pw_threads t RIGHT JOIN $pw_tmsgs tm ON tm.tid=t.tid $sql");
	while($rt=$db->fetch_array($query)){
