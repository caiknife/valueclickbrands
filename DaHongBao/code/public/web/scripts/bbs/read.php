...
//if(!empty($_GET['fpage']) && empty($_GET['page'])){
//	header("HTTP/1.1 301 Moved Permanently");
//	header("Location: read.php?tid=".$_GET['tid']."");
//	exit();
//}


$url = $_SERVER["REQUEST_URI"];
$start = strpos($url, "fpage=");
if($start < 1) {
...
if($page>1){
	...
	$S_sql=',tm.*,m.uid,m.username,m.gender,m.oicq,m.kf,m.groupid,m.memberid,m.icon AS micon ,m.hack,m.honor,m.signature,m.showsign,m.regdate,m.signchange,m.medals,m.payemail,md.postnum,md.digests,md.rvrc,md.money,md.credit,md.currency,md.thisvisit,md.lastvisit,md.onlinetime,md.starttime';
...
$readcategorykey = $db->query("SELECT category FROM pw_category_keywords WHERE fid='$fid' ORDER BY RAND()");
//print_r($read);
while ($rt = $db->fetch_array($readcategorykey)){
	$adkeywords[] = $rt['category'];
}
if($adkeywords) $adkeywords = implode(",",$adkeywords);
...
$guidename = createNewBarArray($read,$guidename,$foruminfo['t_type']);
$nowtype =  $read['type'];
$guidename[$fid]=array($subject,"read.php?tid=$tid");

//增加google搜索关键词 
$categoryNameKeys = @array_slice($guidename, -2, 1);
...
if($read['replies']>0){
	...
	$order = $rewardtype>0 ? "ifreward DESC,postdate" : "postdate";
...
function viewread($read,$start_limit){
	...
	if($read['groupid']!='' && $anonymous==0){
		...
		if($anonymous){
			...
			$read['payemail'] = $read['oicq'] = $read['ip'] = '';
	...
	if($rewardtype){
		if($read['lou']==0 || $read['ifreward']>0 || ($rewardtype=='1' && $winduid==$GLOBALS['authorid'] && $winduid!=$read['authorid'])){
