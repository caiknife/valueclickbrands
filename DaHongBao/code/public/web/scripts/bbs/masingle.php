...
if($action=="banuser"){
	...
	$userdb = $db->get_one("SELECT t.anonymous,m.uid,m.username,m.groupid FROM $sqltab WHERE t.tid='$tid' AND t.fid='$fid' $sqladd");
	...
	if(!$_POST['step']){
		...
		foreach($threaddb as $key=>$val){
			if($val['pid'] == 'tpc'){
				...
				$oUserScoreDao = new UserScoreDao($val['authorid']);
				$oUserScoreDao->addScore("DELETEPOST",$val['tid'],"bbs","","","",$db);
				if($val['aid']){
				...
				require_once("../../lib/dao/class.UserScoreDao.php");
				$oUserScoreDao = new UserScoreDao($val['authorid']);
				$oUserScoreDao->addScore("DELETECOMMIT",$val['tid'],"bbs","","","",$db);
				if($val['aid']){
					$attachs= unserialize(stripslashes($val['aid']));
					foreach($attachs as $key=>$value){
						if($ftp && !file_exists($attachdir."/".$value['attachurl'])){
