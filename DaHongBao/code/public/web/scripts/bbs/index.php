...
$topicarray = HomepageDao::getTopicConfig('BBS');
...
//调用开始
$cachefile=D_P."data/bbscache/indexhotpost.php";
...
unset($members);
$query = $db->query("SELECT tid,author,subject,postdate FROM pw_threads WHERE pw_threads.fid <17 OR pw_threads.fid>53 order by postdate desc limit 0,$listnum");
...
unset($threads);
$query = $db->query("SELECT tid,author,subject,hits FROM pw_threads WHERE (pw_threads.fid <17 OR pw_threads.fid>53) AND pw_threads.fid!=0 order by hits desc limit 0,$listnum");
while($threads=$db->fetch_array($query)){
$threads[subject]=substrs($threads[subject],$listlength);
$hitthreads.="<a href='read.php?tid=$threads[tid]' title='作者:$threads[author] 浏览:$threads[hits]'>$threads[subject]</a><br />";
}
unset($threads);
$query = $db->query("SELECT tid,author,subject,replies,lastpost FROM pw_threads WHERE (pw_threads.fid <17 OR pw_threads.fid>53) AND replies>0 AND pw_threads.fid!=0 order by lastpost desc limit 0,$listnum");
while($threads=$db->fetch_array($query)){
