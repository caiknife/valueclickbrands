...
if($action=='delimg'){
	...
	if(!$_POST['step']){
		...
		foreach($credittype as $key=>$cid){
			if(in_array($cid,array('rvrc','credit'))){
				$creditselect .= '<option value="'.$cid.'">'.${'db_'.$cid.'name'}.'</option>';
			} elseif(is_numeric($cid) && isset($_CREDITDB[$cid])){
				$creditselect .= '<option value="'.$cid.'">'.$_CREDITDB[$cid][0].'</option>';
			}
		}
		if($maxcredit){
		...
		if(in_array($cid,array('rvrc','money','credit'))){
			$name = ${'db_'.$cid.'name'};
			$unit = ${'db_'.$cid.'unit'};
		} elseif(is_numeric($cid) && isset($_CREDITDB[$cid])){
			$name = $_CREDITDB[$cid][0];
			$unit = $_CREDITDB[$cid][1];
		} else{
		...
		if($markdt && $addpoint>0){
			...
			addcredit($winduid,$cid,-$addpoint);
		}
	...
	if($rt){
		$urladd='';
		if($goto=='next'){
			$rt=$db->get_one("SELECT aid,tid FROM pw_attachs WHERE uid='$rt[uid]' AND type='img' AND aid<'$aid' ORDER BY aid DESC LIMIT 1");
			!$rt['aid'] && $urladd='&nonext=1';
		}elseif($goto=='pre'){
			$rt=$db->get_one("SELECT aid,tid FROM pw_attachs WHERE uid='$rt[uid]' AND type='img' AND aid>'$aid' ORDER BY aid LIMIT 1");
			!$rt['aid'] && $urladd='&nopre=1';
		}
		$rt['aid'] && $aid=$rt['aid'];
		ObHeader("show.php?action=pic&tid=$rt[tid]&aid=$aid$urladd");
	}else{
	...
	if($goto=="previous"){
		$next = $db->get_one("SELECT tid,postdate FROM pw_threads WHERE fid='$fid' AND topped='0' AND $by>'$lastpost' ORDER BY $by ASC LIMIT 1");
		if($next) {
		...
		$last = $db->get_one("SELECT tid,postdate FROM pw_threads WHERE fid='$fid' AND topped='0' AND $by<'$lastpost' ORDER BY $by DESC LIMIT 1");
		if($last) {
	...
	*/
	...
	if($fgeturl[0]){
		if($fgeturl[1]=='Local'){
			$fgeturl[0]=__IMAGE_SRC.$fgeturl[0];
	...
	*/
	...
	if($pid == 'tpc'){
		$table = GetTtable($tid);
		$tpcs = $db->get_one("SELECT authorid,content,buy FROM pw_threads t LEFT JOIN $table tm ON tm.tid=t.tid WHERE t.tid='$tid'");
	}elseif(is_numeric($pid)){
		$table = GetPtable('N',$tid);
		$tpcs = $db->get_one("SELECT authorid,content,buy FROM $table WHERE pid='$pid' && tid='$tid'");
	}
	...
	$buy=$tpcs['buy'].",".$windid;
	$db->update("UPDATE $table SET buy='".addslashes($buy)."' WHERE tid='$tid'");
	...
	*/
	...
	if(!$step){
		...
		if($type=='1'){
			...
			$rw_a_val<1 && Showmsg('reward_help_error');
			$rw_a_val--;
			$rewardinfo = "1\n".$rw_b_name.'|'.$rw_b_val.'|'.$rw_a_name.'|'.$rw_a_val;
			$db->update("UPDATE pw_threads SET rewardinfo='$rewardinfo' WHERE tid='$rt[tid]'");
			$db->update("UPDATE $pw_posts SET ifreward='1' WHERE pid='$pid'");
			addcredit($rt['authorid'],$rw_a_name,1);  /*热心助人者加分*/
		}
		if($ifmsg){
			require_once(R_P.'require/msg.php');
			include_once(D_P.'data/bbscache/creditdb.php');
			...
			if($type=='1'){
				...
				$creditname = is_numeric($rw_a_name) ? $_CREDITDB[$rw_a_name][0] : ${'db_'.$rw_a_name.'name'};
				$affect = $creditname.":1";
			}
...
function addcredit($uid,$cid,$addpoint){
	global $db;
	if(in_array($cid,array('rvrc','money','credit'))){
		$cid == 'rvrc' && $addpoint *= 10;
		$db->update("UPDATE pw_memberdata SET $cid=$cid+'$addpoint' WHERE uid='$uid'");
...
function addcredits($uids,$cid,$addpoint){
	...
	if(in_array($cid,array('rvrc','money','credit'))){
		$cid == 'rvrc' && $addpoint *= 10;
		$db->update("UPDATE pw_memberdata SET $cid=$cid+'$addpoint' WHERE uid IN($uids)");
	} elseif(is_numeric($cid)){
		$uid_update=array();
...
function return_value($tid){
	...
	$query=$db->query("SELECT pid,authorid FROM $pw_posts WHERE tid='$tid' AND ifreward='0' GROUP BY authorid ORDER BY postdate ASC LIMIT $rw_a_val");
	$pids = $uids='';
	...
	$pids && $db->update("UPDATE $pw_posts SET ifreward='1' WHERE pid IN($pids)");
}
?>