...
if($action == "type"){
	...
	if(empty($_POST['step'])){
		...
		$delids  = 0;
		$readdb  = $ttable_a = $ptable_a = array();
		foreach($tidarray as $key => $value){
			if(is_numeric($value)){
				$delids .= ','.$value;
		...
		$deletetid = explode(",",$delids);
		require_once("../../lib/dao/class.UserScoreDao.php");
		foreach ($deletetid as $key=>$value){
			if($value>0){
				$whopost = $db->get_one("SELECT authorid FROM pw_threads WHERE tid='$value'");
				
				$oUserScoreDao = new UserScoreDao($whopost['authorid']);
				$oUserScoreDao->addScore("DELETEPOST",$value,"bbs","","","",$db);

				$query=$db->query("SELECT tid,authorid FROM pw_posts WHERE tid='$value'");
				if($query){
					while($rt = $db->fetch_array($query)){
						$tid = $rt['tid'];
	...
	if(empty($_POST['step'])){
		$fromdb=$db->get_one("SELECT * FROM pw_threads WHERE tid='$seltid'");
		...
		$ttable_a[GetTtable($totid)] .= $totid.',';
		foreach($ttable_a as $pw_tmsgs=>$val){
			$val = substr($val,0,-1);
		...
		$pw_tmsgsf = GetTtable($fromtid);
		$db->update("DELETE FROM $pw_tmsgsf WHERE tid='$fromtid'");
