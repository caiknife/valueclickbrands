...
require_once($_SERVER['DOCUMENT_ROOT']."/etc/const.inc.php");
...
if($_SERVER['HTTP_X_FORWARDED_FOR']){
	...
	$c_agentip=1;
}elseif($_SERVER['HTTP_RLNCLIENTIPADDR']){
	$onlineip = $_SERVER['HTTP_RLNCLIENTIPADDR'];
	$c_agentip=1;
}else{
...
//echo __IMAGE_ADD;
$imgdir		= R_P.$picpath;
$attachdir	= __IMAGE_ADD.$attachname;
$pw_posts   = 'pw_posts';
$pw_tmsgs   = 'pw_tmsgs';
...
function refreshto($URL,$content,$statime=1,$downimage=FALSE){
	global $db_ifjump;
	$URL=str_replace('&#61;','=',$URL);
	if($db_ifjump && $statime>0){
		...
		if($downimage) $lang[$content] = "&nbsp;<span style='color:#FF0000'>您发表内容中的图片已经成功下载</span>&nbsp;<BR>".$lang[$content];
...
function footer(){
	global $db,$db_obstart,$db_footertime,$db_htmifopen,$P_S_T,$mtablewidth,$db_ceoconnect,$wind_version,$imgpath,$stylepath,$footer_ad,$db_union,$timestamp,$db_icp,$db_icpurl,$db_siteifopen,$advertdb,$db_yh;
	Update_ol();
	if($db){
		$qn=$db->query_num;
	}
...
function AdvertInit($SCR,$fid){
	...
	$newadvert = array();
	if(empty($advertdb)) return $newadvert;
...
//add by eric_chu for new bar
function createNewBarArray($get,$oarray,$f_type){
	if(empty($get['type'])) return $oarray;
	$t_typedb =explode("\t",$f_type);/*主题分类*/
	if(array_key_exists('type',$get)){
		$t = $get['type'];
		$oarray[] = array("0"=>$t_typedb[$t],"1"=>"thread.php?fid=".$get['fid']."&type=".$t);
	}
