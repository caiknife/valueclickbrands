...
function bbspostguide(){
	global $db,$creditset,$db_creditset,$db_upgrade,$db_hour,$ifupload,$groupid,$windid,$winduid,$winddb,$timestamp,$top_post,$fatherid,$fid,$tid,$tdtime,$db_autochange,$db_tcheck,$atc_content,$gp_allowupload;
	...
	if($groupid != 'guest'){
		...
		if($ifupload && $gp_allowupload==1 && $Globals['uploadmoney']){
			global $uploadmoney,$uploadcredit;
...
function check_data($type="new"){
	global $atc_usesign,$article,$db_titlemax,$db_postmin,$db_postmax,$atc_money,$foruminfo,$db_wordsfb;
	...
	if(strlen($_POST['atc_content'])>=$db_postmax || strlen(trim($_POST['atc_content']))<$db_postmin){
	...
	$atc_content = wordsfb($_POST['atc_content']);
	$ifwordsfb   = $atc_content == $_POST['atc_content'] ? $db_wordsfb : 0;
	$ifconvert   = 1;
...
function postupload($tmp_name,$filename){
	...
	if(function_exists("move_uploaded_file") && @move_uploaded_file($tmp_name,$filename)){
		...
		$t = explode(__IMAGE_ADD,$filename);
		FileDistribute::syncImages($t[1]);
		return true;
