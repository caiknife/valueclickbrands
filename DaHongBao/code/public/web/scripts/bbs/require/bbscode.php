...
function tablefun($width,$unit,$text){
	...
	$td_num = 0;
	$td_htm = array();
	$text = str_replace(array('\\"','[tr]','[/tr]'),array('"','<tr class="tr3">','</tr>'),$text);
	$text = preg_replace(
		array('/\[td=(\d{1,2}),(\d{1,2})(,(\d{1,3}%?))?\](.*?)\[\/td\]/eis','/\[td\](.*?)\[\/td\]/eis'),
		array("tdfun('\\5','\\1','\\2','\\4')","tdfun('\\1')"),$text
	);
	$text = preg_replace('/\[.+?\]/is','',$text);
	krsort($td_htm);
	foreach($td_htm as $key=>$value){
		$text = str_replace("<\ttd_code_$key\t>",$value,$text);
	}
	return "<table style=\"border:1px solid $tdcolor;width:$width\">$text</table>";
}
function tdfun($text,$col='',$row='',$width=''){
	global $td_num,$td_htm;
	$td_num ++;
	$td_htm[$td_num] = ($col && $row) ? "<td colspan=\"$col\" rowspan=\"$row\" width=\"$width\">" : "<td>";
	$td_htm[$td_num].= str_replace('\\"','"',$text).'</td>';
	return "<\ttd_code_$td_num\t>";
}
...
function cvpic($url,$type='',$picwidth='',$picheight=''){
	...
	if(substr($lower_url,0,4)!='http')$url=__IMAGE_SRC."$url";
	...
	if($user_a[1]){
		if(!ereg("^http",$user_a[1])){
			if($db_ftpweb && !file_exists($attachdir.'/upload/'.$user_a[1])){
				...
				$user_a[1] = __IMAGE_SRC.$attachpath.'/upload/'.$user_a[1];
			}
		}
