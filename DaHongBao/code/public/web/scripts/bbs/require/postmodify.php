...
if(!$_POST['step']){
	...
	if($article == 0){
		...
		$oUserScoreDao = new UserScoreDao($authorid);
		$oUserScoreDao->addScore("DELETECOMMIT",$tid,"bbs","","","",$db);
	...
	*/
	...
	if($attachs){
		...
		$gp_allowupload==1 && $winddb['money'] += $uploadmoney;
	...
	if($article == 0){
		...
		$db->update("UPDATE $pw_tmsgs SET aid='$oldattach',$ipdata ifsign='$atc_usesign',alterinfo='$alterinfo',ifconvert='$ifconvert',ifwordsfb='$ifwordsfb',content='$atc_content' WHERE tid='$tid'");
		if($aids){
		...
		$db->update("UPDATE $pw_posts SET aid='$oldattach',$ipdata icon='$atc_iconid',subject='$atc_title',ifsign='$atc_usesign',alterinfo='$alterinfo',ifconvert='$ifconvert',ifwordsfb='$ifwordsfb',content='$atc_content',anonymous='$anonymous' WHERE pid='$pid'");
		if($aids){
	...
	if($modify){
		...
		refreshto("read.php?tid=$tid&page=$page&toread=1",'enter_thread','1',$glbDownloadImage);
	}
}
?>