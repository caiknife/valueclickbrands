...
$phpwind = new PHPWIND();
$tpl = new sTemplate();
$tpl->setTemplate("new/mydiscount.htm");
$page = $_GET['page'];
...
if ($_GET['switch'] == 'myadddiscount') {
	$userdiscountlist = $phpwind->getUserDiscount($userinfo[0]['uid'], $page);
	$tpl->assign("userdiscountlist", $userdiscountlist);

	$userdiscountlistCount = $phpwind->getUserDiscountCount($userinfo[0]['uid']);

	$pageCount = ceil($userdiscountlistCount / 10);
	$pageStr = getNewPageStr($page, $pageCount, '', 'myadddiscount');
	$tpl->assign("pageStr", $pageStr);
}
elseif ($_GET['switch'] == 'myfavordiscount') {
	$userfavordiscountlist = $phpwind->getUserFavorDiscount($userinfo[0]['uid'], $page);
	$tpl->assign("userfavordiscountlist", $userfavordiscountlist);

	$getUserFavorDiscountCount = $phpwind->getUserFavorDiscountCount($userinfo[0]['uid']);

	$pageCount = ceil($getUserFavorDiscountCount / 10);
	$pageStr = getNewPageStr($page, $pageCount, '', 'myfavordiscount');
	$tpl->assign("pageStr", $pageStr);

}elseif ($_GET['switch'] == 'myuseradddiscount') {
	$useradddiscountlist = $phpwind->getUserAddDiscount($userinfo[0]['uid'], $page);
	$tpl->assign("useradddiscountlist", $useradddiscountlist);

	$getUserFavorDiscountCount = $phpwind->getUserAddDiscountCount($userinfo[0]['uid']);
