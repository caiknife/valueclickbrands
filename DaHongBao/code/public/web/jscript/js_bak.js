function add_bookmark(){
   var url="http://www.couponmountain.com"
   var title="www.couponmountain.com"
   if ((navigator.appName == "Microsoft Internet Explorer") && (parseInt(navigator.appVersion) >= 4)){
      if (confirm('Add couponmountain.com to your favorite?')){
...
function popdown(serverroot,slist) {
   if (!getCookie("popdown")){
      var today = new Date();
      setCookie("popdown","popdownvalue","/",".couponmountain.com",new Date(today.getTime() + (23-today.getUTCHours())*3600000));
...
function orig_make_stat(category,merchant,coupon,is_frame){
   ...
   if ( !CSession ){
      newsess = 1;
      setCookie("SESSION","1","/",".couponmountain.com");
   }
...
//   if ( referer && (referer != document.location.hostname) ){
   if ( referer && ( referer.indexOf("couponmountain.com") < 0 ) ){
   ...
   setCookie('REFERER',referer,"/",".couponmountain.com",new Date(today.getTime() + 604800000000));
   setCookie('KEYWORD',keyword,"/",".couponmountain.com",new Date(today.getTime() + 604800000000));
   ...
   setCookie('SOURCE',source,"/",".couponmountain.com",new Date(today.getTime() + 604800000000));
...
function make_stat(category,merchant,coupon,is_frame){
   ...
   setCookie('REFERER',referer,"/",".couponmountain.com",new Date(today.getTime() + 2592000000));
   setCookie('KEYWORD',keyword,"/",".couponmountain.com",new Date(today.getTime() + 2592000000));
...
function set_source() {
   ...
   setCookie('SOURCE',source,"/",".couponmountain.com",new Date(today.getTime() + 2592000000));
   setCookie('SOURCEGROUP',sourcegroup,"/",".couponmountain.com",new Date(today.getTime() + 2592000000));
   return source;
...
function make_frames(page, merchant, frm, m_id, m_name) {
    var source = find_source();
    if (m_name){
      window.status='Merchant Page - '+m_name+' Coupons - Updated Daily!';
    }
    else{
      window.status='Thank you for visiting CouponMountain.';
    }
    if( merchant != '') {
...
function afp_stat() {
    ...
    // for couponmountain.com/jscript/cookie.js)
    var afpie = getCookie("AFP");

    var afp_statscript = "http://affiliates.couponmountain.com/afp_altclick.php";
    ...
    if( afpie != "1" ) {
        ...
        setCookie("AFP", "1", "/", ".couponmountain.com");
        ...
        if (url.indexOf('crid=') > 0 && url.indexOf('afid=') > 0) {
            ...
            setCookie("CRID", crid, "/", ".couponmountain.com");
            setCookie("AFID", afid, "/", ".couponmountain.com");
        }
    }
}
