...
<link href="../css/search_allbd_yh.css" rel="stylesheet" type="text/css" media="all" />
...
<div id="content">
	<!--头部开始-->
	<div class="head_yh">
		<div class="head_yhleft"><FORM METHOD=GET action="/search.php" NAME="searchform" target="_parent" onsubmit="if(searchform.searchText.value=='' || searchform.searchText.value=='请输入关键字') return false;">
			<ul>
				<li class="lilogo"><a href="/"><img src="/images/newsearch/logo.gif" alt="大红包" /></a></li>
				<li class="lileft"><input name="searchText" type="text" style="{if $searchText && $searchText != '请输入关键字'}color:#000;{else}color:#CCC;{/if}" value="{if $searchText}{$searchText}{else}请输入关键字{/if}" class="input" {literal}onblur="if(this.value=='') { this.value='请输入关键字'; this.style.color='#CCC'; }else{ this.style.color='#000'; }"{/literal} onfocus="if(this.value=='请输入关键字') this.value=''; this.style.color='#000';"/><INPUT TYPE="hidden" name="needjump" value="true"/></li>
				<li class="licenter"><input type="image" src="/images/newsearch/search.gif" alt="搜索" /></a></li>
				<li class="liright"><input type="image" src="/images/newsearch/search_text.gif" alt="优惠折扣大搜索" /></li>
			</ul></FORM>
		</div>
		<div class="head_yhright">
			<ul>
				{if $islogon==1}
				<li>你好，<a href="/bbs/profile.php">{$userinfo.username}</a> <a href="/bbs/login.php?action=quit">退出</a></li>
				<li>目前红包数：{$userinfo.money}个 <a href="/profile/myhongbao.php">我的大红包</a></li>
				{else}
				<li>&nbsp;</li>
				<li><a href="/bbs/login.php">登录</a>&nbsp;<a href="/bbs/register.php">注册</a></li>
				{/if}
			</ul>
		</div>
	</div>
