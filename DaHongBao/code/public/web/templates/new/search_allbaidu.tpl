...
<div id="content">
	...
	<div class="topad">
		...
		<ul>
			...
			<li class="li_text">{$l->abstract}</li>
			<li class="li_url"><a href="{$l->uri}" target="_blank">{$l->SHORTURL}</a> - <a href="{$l->uri}" target="_blank"><strong>推广</strong></a></li>		
		</ul>
		{/if}
		{/foreach}
	</div>
			...
			<div class="coupon_left">				
				<div class="discount_box1">
					<ul>
						...
						<!-- <li class="litime_city">发表时间：{$newsearchlist[loop].createdTime}</li> -->
					</ul>
				</div>
			</div>
			<!--同城生活内容结束-->
	...
	<div id="content_right">
		{if count($adsbd->resultlist->result) > 2}
		<div id="preview" class="content_rightadd">
			<div class="bd_title"><img src="/images/ico_bd.gif" /></div>
			{foreach from=$adsbd->resultlist->result item=l name=list}
		{if $smarty.foreach.list.index > 1 && $smarty.foreach.list.index <8}
			<ul>
				<li class="lititle"><a href="{$l->uri}" target="_blank">{$l->title}</a></li>
				<li class="litext">{$l->abstract}</li>
				<li class="liurl"><a href="{$l->uri}" target="_blank">{$l->SHORTURL}</a> - <a href="{$l->uri}" target="_blank"><strong>推广</strong></a></li>
			</ul>
			{/if}			
			{/foreach}			
		</div>
