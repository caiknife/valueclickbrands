...
class Module
{
    ...
    public function setDefaultView($event)
    {
        ...
        $this->siteConfg = $config['siteUrlConfig']($_SERVER['HTTP_HOST']);
        $this->siteConfg['couponUrl'] = PathManager::getDhbCouponUrl(); //优惠券
        $this->siteConfg['dealsUrl']  = PathManager::getDhbDealsUrl();  //精选促销

        // 全部优惠券分类
        $sm = $event->getApplication()->getServiceManager();
        $categoryTable   = $sm->get('Category');
        $allCategoryList = $categoryTable->getCategoryList("1");
        foreach ($allCategoryList as $k => $category) {
            $allCategoryList[$k]['CategoryUrl'] = PathManager::getDhbCouponCateUrl($category['CategoryID']);
        }
        $this->siteConfg['allCategoryList'] = $allCategoryList;
